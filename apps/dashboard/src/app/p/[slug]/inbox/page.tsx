...
export default function ProjectInboxPage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold">💡 Инбокс идей</h1>
        <p className="text-gray-500 text-sm mt-1">
          Собирайте идеи — запускайте в прогон одним кликом
        </p>
      </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-gray-600">
          ...
          {idea.source && idea.source !== 'manual' && (
            <span className="ml-2 text-gray-500">· {idea.source}</span>
          )}
        </p>
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={launching || idea.status === 'in_progress'}
          ...
          >
            {launching ? '…' : idea.status === 'in_progress' ? '⚡ Запущено' : '🚀 В прогон'}
          </button>
        </div>
      </div>
    </div>
