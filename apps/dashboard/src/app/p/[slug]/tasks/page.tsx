...
import { getDrafts, updateDraft, deleteDraft, publishDraftToTelegram } from '@/lib/api';
...
export default function ProjectTasksPage() {
  ...
  async function handleSave(draft: ContentDraft) {
    setSaving(draft.id);
    try {
      const updated = await updateDraft(draft.id, { content: editContent[draft.id] ?? draft.content });
  ...
  async function handlePublishTelegram(draft: ContentDraft) {
    if (!confirm(`Опубликовать «${draft.title}» в Telegram-канал?`)) return;
    setPublishing(draft.id);
    try {
      await publishDraftToTelegram(draft.id);
      setDrafts((prev) => prev.map((d) => d.id === draft.id ? { ...d, status: 'published' as ContentDraft['status'] } : d));
      alert('✅ Опубликовано в Telegram-канал!');
    } catch (e) { alert((e as Error).message); }
            ...
            {expanded === draft.id && (
              <div className="border-t border-gray-700 p-4 space-y-3">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <button
                    onClick={() => handlePublishTelegram(draft)}
                    disabled={publishing === draft.id || draft.status === 'published'}
                    className="bg-sky-700 hover:bg-sky-600 disabled:opacity-40 text-white px-4 py-2 rounded-lg text-sm transition-colors"
                    title="Опубликовать в Telegram-канал"
                  >
                    {publishing === draft.id ? 'Публикую…' : '✈ Telegram'}
                  </button>
                  <button
